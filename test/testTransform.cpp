...
TEST_F(TransformFixture, scale_defaults_to_identity) {
    cal3d::Scale s;
    CHECK_EQUAL(CalVector(1, 1, 1), s.scale);
}

TEST_F(TransformFixture, can_assign_vector_to_scale) {
    cal3d::Scale s;
    s = CalVector(1, 2, 3);
    CHECK_EQUAL(CalVector(1, 2, 3), s.scale);
}

TEST_F(TransformFixture, matrix_is_column_major) {
    CalMatrix matrix(CalVector(0, 1, 0), CalVector(-1, 0, 0), CalVector(0, 0, 1));
    CHECK_EQUAL(CalVector(0, 1, 0), matrix * CalVector(1, 0, 0));
}

TEST_F(TransformFixture, non_uniform_scale_before_and_after_translation) {
...
TEST_F(TransformFixture, non_uniform_scale_before_and_after_transform) {
    cal3d::Scale scale(CalVector(1, 2, 3));
...
TEST_F(TransformFixture, can_convert_close_to_180_rotation_matrix_to_quaternion) {
