...
#ifdef WIN32
#define PLATFORM_HAS_STRNICMP
#else
#undef PLATFORM_HAS_STRNICMP
#endif
...
#endif
        returnNode = NewTiXmlDeclaration();
    } else if (isalpha(*(p + 1))
               || *(p + 1) == '_') {
#ifdef DEBUG_PARSER
        TIXML_LOG("XML parsing Element\n");
#endif
        returnNode = NewTiXmlElement();
    } else if (StringEqual(p, commentHeader, false)) {
#ifdef DEBUG_PARSER
        TIXML_LOG("XML parsing Comment\n");
#endif
        returnNode = NewTiXmlComment();
    } else {
#ifdef DEBUG_PARSER
        TIXML_LOG("XML parsing Unknown\n");
#endif
        returnNode = NewTiXmlUnknown();
    }
...
const char* TiXmlElement::Parse(const char* p, TiXmlParsingData* data) {
    ...
    if (!p || !*p) {
        if (document) {
            document->SetError(TIXML_ERROR_PARSING_ELEMENT, 0, 0);
        }
        return 0;
    }
...
//	TiXmlParsingData data( p, prevData );
    ...
    if (*p != '<') {
        if (document) {
            document->SetError(TIXML_ERROR_PARSING_ELEMENT, p, data);
        }
        return 0;
    }
    ...
    while (p && *p) {
        ...
        if (*p == '/') {
            ...
            if (!p || !*p) {
                ...
                ReturnToFactory(attrib);
                return 0;
            }

            // Handle the strange case of double attributes:
            char const* name = attrib->Name();
...
const char* TiXmlElement::ReadValue(const char* p, TiXmlParsingData* data) {
    ...
    while (p && *p) {
        if (*p != '<') {
            ...
            TiXmlText* textNode = NewTiXmlText();
...
//	TiXmlParsingData data( p, prevData );
    ...
    if (!p || !*p || *p != '<') {
        if (document) {
            document->SetError(TIXML_ERROR_PARSING_UNKNOWN, p, data);
        }
        return 0;
    }
    ++p;
    ...
    if (!p) {
        if (document)	{
            document->SetError(TIXML_ERROR_PARSING_UNKNOWN, 0, 0);
        }
    }
    if (*p == '>') {
        return p + 1;
