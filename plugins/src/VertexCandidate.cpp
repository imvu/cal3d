...
float CVertexCandidate::EPSILON = 0.01f;
...
bool CVertexCandidate::operator==(CVertexCandidate& vertexCandidate)
{
...
#if 0
	// compare the normal
	if((fabs(m_normal[0] - vertexCandidate.m_normal[0]) > 0.1)
		|| (fabs(m_normal[1] - vertexCandidate.m_normal[1]) > 0.1)
		|| (fabs(m_normal[2] - vertexCandidate.m_normal[2]) > 0.1)) 
		return false;
#endif

	// compare the texture coordinates
	if(m_vectorTextureCoordinate.size() != vertexCandidate.m_vectorTextureCoordinate.size()) 
		return false;
	...
	for(size_t textureCoordinateId = 0; textureCoordinateId < m_vectorTextureCoordinate.size(); textureCoordinateId++)
	{
		if((fabs(m_vectorTextureCoordinate[textureCoordinateId].u - vertexCandidate.m_vectorTextureCoordinate[textureCoordinateId].u) > EPSILON)
			|| (fabs(m_vectorTextureCoordinate[textureCoordinateId].v - vertexCandidate.m_vectorTextureCoordinate[textureCoordinateId].v) > EPSILON)) 
			return false;
	}
