...
#if CAL3D_EXPORT_OPTION_DESELECT_DUMMY_NODES
	m_bSelected = !theExporter.GetInterface()->IsDummy(pNode);
#else
	m_bSelected = true;
#endif

#if CAL3D_EXPORT_OPTION_DESELECT_EX_NODES
  std::string name = pNode->GetName();
  static char prefix[] = CAL3D_EXPORT_OPTION_DESELECT_EX_NODES_PREFIX_STRING;
  if( !_strnicmp(prefix, name.c_str(), strlen(prefix)) ) {
    m_bSelected = false;
  }
#endif
